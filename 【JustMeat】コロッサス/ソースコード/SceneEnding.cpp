...
#include "keyboard.h"
#include "AudioData.h"
SceneEnding::SceneEnding(SceneManager * sceneManager, Parameter param)
	: Scene(sceneManager)
{
	StopBGM(0);
	StopBGM(1);
	...
	auto it = new ScreenBackground(this, TextureTag::Plane);
	it->GetCASprite()->SetColor(0xFFFFFFFF);
	LoadMessage();
	NextText();
	...
	mEnter->GetCSprite()->SetStringColor(0xFFFFFFFF);

}

void SceneEnding::Update(float deltaTime)
{
	...
	if (Keyboard::Inst()->KeyHit(DIK_RETURN))
	{
		...
		{
			Parameter param;
			GetSceneManager()->SceneChange(SceneManager::SceneID::eCredit, &param, 2.0f, 2.0f, 2.0f, true);
			PlayBGM(BGM_ENDING, 0, 0, false);
			SetFadein(0, 25, 0);
		}
	}
}

void SceneEnding::LoadMessage()
{
	FILE* fp = fopen("resources/ending.txt", "r");
...
bool SceneEnding::NextText()
{
	...
	for (int i = 0; i < mTexts[mCurrentMessage].size(); i++)
	{
		...
		text->GetCSprite()->SetStringColor(0xFF000000);
		mTotalHeight += height;
	}
	mCurrentMessage++;
	return false;
